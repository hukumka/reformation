...
pub fn attribute_unknown_mode(span: Span, name: &str) -> Error {
    let msg = format!("Unknown mode `{}`.", name);
    Error::new(span, msg)
}
...
pub fn format_string_enum_start(span: Span, c: Option<char>) -> Error {
    ...
    if let Some(c) = c {
        ...
        msg = "Enum format string must be \"(variant1|variant2|...|variantN)\", but it is empty."
            .to_string();
    }
    Error::new(span, msg)
}
...
pub fn named_struct_argument_with_no_field(span: Span, name: &str) -> Error {
    let msg = format!(
        "Format string contains argument {{{0}}}, but no corresponding field `{0}` found.",
        name
    );
    Error::new(span, msg)
}
...
pub fn unnamed_struct_wrong_argument_count(span: Span, real: usize, expected: usize) -> Error {
    let msg = format!(
        "Struct contains {} fields, but format string has {}.",
        expected, real
    );
    Error::new(span, msg)
}
...
pub fn enum_variant_wrong_number_of_values(
    span: Span,
    ident: &Ident,
    real: usize,
    expected: usize,
