...
use syn::{
    Ident, Generics, Type, Attribute, Data, DataStruct, DataEnum,
...
use crate::reformation_attribute::{ReformationAttribute, Modes};
...
impl DeriveInput{
    ...
    pub fn generics(&self) -> &Generics { &self.generics }
    pub fn arguments(&self) -> &Arguments { &self.arguments }
...
impl ArgumentsNamed{
    ...
    pub fn default_fields(&self) -> (Vec<&Ident>, Vec<&Type>){
        let (names, types): (Vec<_>, Vec<_>) = self.defaults.iter()
            .unzip();
        (names, types)
    }
}
...
impl StructFormat{
    fn parse(span: Span, attrs: Vec<Attribute>) -> syn::Result<Self>{
        ...
        let format = Format::new(&attr.regex_string)
...
impl EnumFormat{
    fn parse(span: Span, attrs: Vec<Attribute>) -> syn::Result<Self>{
        ...
        let format: syn::Result<Vec<_>> = variants.into_iter()
            .map(|s|{
                let format = Format::new(s)
                    .map_err(|e| errors::format_error(span, e))?;
                let format = apply_modes(format, &attr.modes);
                Ok(format)
            })
...
impl Arguments{
    ...
    fn parse_enum(enum_: DataEnum, format: &EnumFormat) -> syn::Result<Self>{
        ArgumentsCases::parse(enum_, format)
            .map(|x| Arguments::Cases(x))
    }
}
...
impl ArgumentsCases{
    fn parse(enum_: DataEnum, format: &EnumFormat) -> syn::Result<Self>{
        ...
        if enum_variant_count != format_variant_count{
            return Err(errors::enum_wrong_number_of_variants_covered(format.span, format_variant_count, enum_variant_count));
        }
        let res: syn::Result<Vec<_>> = enum_.variants.iter()
            .zip(&format.format)
...
impl EnumVariant{
    fn parse(span: Span, variant: &Variant, format: &Format) -> syn::Result<Self>{
        let fields: Vec<_> = variant.fields.iter()
            .map(|x| x.clone().ty)
            .collect();
        let expected = fields.len();
        let real = format.positional_arguments();
        if expected != real{
            return Err(errors::enum_variant_wrong_number_of_values(span, &variant.ident, real, expected));
        }
...
impl ArgumentsPos{
    fn parse(struct_: FieldsUnnamed, format: &StructFormat) -> syn::Result<Self>{
        let fields: Vec<_> = struct_.unnamed.into_iter()
            .map(|x| x.ty)
            .collect();
        let real = fields.len();
